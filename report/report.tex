...
\fancyhead[L]{\small\color{midgray}\textbf{EV Charging Demand Prediction}}
...
% Content sits below the top bar
\vspace*{1.2cm}
\begin{center}
  {\small\bfseries\color{white}
    \MakeUppercase{Applied Machine Learning \quad\textperiodcentered\quad Team Project}}\\[0.3cm]
  {\footnotesize\color{accentlight}
    \MakeUppercase{Mid-Semester Submission \quad\textperiodcentered\quad Milestone 1}}
\end{center}
...
\begin{center}
\begin{tikzpicture}
  \fill[mainblue] (0,0) circle (0.9cm);
  \node at (0,0) {\color{white}\fontsize{22}{22}\selectfont $\boldsymbol{\lightning}$};
\end{tikzpicture}
\end{center}
...
% Main title
\begin{center}
  {\fontsize{28}{36}\selectfont\bfseries\color{darkblue}
    Intelligent EV Charging\\[0.25em]
    Demand Prediction \&\\[0.25em]
    Agentic Infrastructure\\[0.25em]
    Planning
  }\\[0.8cm]
  {\large\itshape\color{midgray}
    Milestone 1 --- Data Pipeline \& Machine Learning Modeling
  }\\[0.8cm]
  \textcolor{mainblue}{\rule{5cm}{2pt}}
\end{center}
...
\begin{tikzpicture}
  \node[
    ...
    \centering
    {\small\bfseries\color{midgray}\MakeUppercase{Project Team}}\\[10pt]
    ...
    \begin{tabular}{@{}l@{\hspace{2em}}l@{}}
      ...
      \textcolor{mainblue}{\textbf{Aditya Rana}}   & Performance Evaluation \& Development \\
    \end{tabular}
  };
\end{tikzpicture}
\end{center}
...
\begin{tikzpicture}
  \node[
    ...
    \color{white}\small
    \textbf{Submission:} Mid-Semester
    \quad\textcolor{accentlight}{\textperiodcentered}\quad
...
\end{tikzpicture}
\end{center}
{\centering\small\itshape\color{midgray} Figure 1 --- End-to-end ML pipeline.\par}
\vspace{0.5em}
...
values were filled using column-wise \textbf{median imputation}, robust to the outliers
visible in electricity price and grid stability distributions. The categorical
\texttt{Grid Availability} feature was binary-encoded:
...
rolling mean shifted by 1 to avoid data leakage. Rows containing NaN values were
then dropped.
...
well-suited due to their robustness to outliers and intrinsic feature importance scoring.
...
\rowcolor{lightgray}
Aditya Rana   & 5      & Model evaluation, metrics reporting, feature importance, development & deployment \\
\bottomrule
\end{tabularx}
\end{table}
